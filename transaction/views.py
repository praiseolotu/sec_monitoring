...
def addTransaction(user,payment_type,total,cart,value):
    ...
    cart_string =  "\n".join(list(cart_df.apply(
                            lambda row: f"{str(row.name)+')':<3}{row['name'][:28]}|".ljust(settings.RECEIPT_CHAR_COUNT)+ "\n"+
                                            f"{row['barcode']:>12}| {row['quantity']:3}| {row['price']:>7}| {row['tax']:>2}".rjust(settings.RECEIPT_CHAR_COUNT),axis=1)))
    cart_string = "NAME | BARCODE| QTY| PRICE| TAX".rjust(settings.RECEIPT_CHAR_COUNT) + f"\n{'-'*settings.RECEIPT_CHAR_COUNT}\n" + cart_string
    
    cart_string = f"Transaction ID:{transaction_id}".center(settings.RECEIPT_CHAR_COUNT) + f"\n{'-'*int(settings.RECEIPT_CHAR_COUNT)}\n" + cart_string
    cart_string = f"Transaction Date:{transaction_date}".center(settings.RECEIPT_CHAR_COUNT) + f"\n{'-'*int(settings.RECEIPT_CHAR_COUNT)}\n" + cart_string
    cart_string = f"Posted On:{posted_on}".center(settings.RECEIPT_CHAR_COUNT) + f"\n{'-'*int(settings.RECEIPT_CHAR_COUNT)}\n" + cart_string
    ...
    total_string = total_string + "\n" + f"{str(payment_type):>10}: N {round(value,2):.2f}".rjust(settings.RECEIPT_CHAR_COUNT)
    total_string = total_string + "\n" + f"{'CHANGE':>10}: N {round(value-total,2):.2f}".rjust(settings.RECEIPT_CHAR_COUNT)
